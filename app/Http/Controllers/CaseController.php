...
class CaseController extends Controller
{
    ...
    public function bulkAdd(Request $request)
    {
        $title = 'Add Bulk News';
        $caseDetails = '';
        return view('case.bulkdata', compact(['title']));
    }
    public function bulkUpload(Request $request)
    {
        ...
        if($ext=='csv'){
            ...
            $fileName = time().'.'.$ext;
            Storage::disk('tmp')->put($fileName,file_get_contents($fileUpload));
            ...
            while($columns=fgetcsv($file)){
                ...
                $officeUnitId=$data['office_unit_id'];
                $caseYear=$data['case_year'];
                $caseNo=$data['case_no'];
                $categoryId=$data['case_category'];
                $caseType=$data['case_type'];
                $thanaId=$data['thana_id'];
                $caseDate=$data['case_date'];
                $complainantName=$data['complainant_name'];
                $complainantMobile=$data['complainant_mobile'];
                $defenderName=$data['defender_name'];
                $defenderMobile=$data['defender_mobile'];

                $caseDetails= new caseDetails();
                $caseDetails->id = uniqid();
                $caseDetails->office_id = $officeId;
                $caseDetails->office_unit_id = $officeUnitId;
                $caseDetails->case_year = $caseYear;
                $caseDetails->case_no = $caseNo;
                $caseDetails->case_category = $categoryId;
                $caseDetails->case_type = $caseType;
                $caseDetails->thana_id =  $thanaId;
                $caseDetails->case_date = $caseDate;
                $caseDetails->complainant_name = $complainantName;
                $caseDetails->complainant_mobile = $complainantMobile;
                $caseDetails->defender_name = $defenderName;
                $caseDetails->defender_mobile = $defenderMobile;
                $caseDetails->save();
                
            } 
            return redirect(config('app.url').'case')->with('success','মামলা গুলো সংরক্ষণ করা হয়েছে');
